...
const storage = multer.diskStorage({
  ...
  filename: async function (req, file, cb) {
      ...
      const newID = maxID + 1;
      const newName = `moviesimage${newID}${ext}`;
      cb(null, newName);
      req.newMovieID = newID; // Attach the new ID to the request object
  }
});
...
app.post('/movies/add', upload.single('image'), async (req, res) => {
  ...
  const movieData = {
      ID: req.newMovieID, // Use the new movie ID generated by multer
      Name: name,
      ...
      ImageUrl: `/Images/moviesimage/moviesimage${req.newMovieID}${path.extname(req.file.originalname)}`
  };
